...
> ⚠️ **NOTA (2026-07-02): el proyecto se reorganizó por componente.** Muchas rutas de
> este documento son de la estructura ANTERIOR (`./scripts/…`, `./docker/…`,
> `/home/lab/pbd_system`). La estructura actual está en `../README.md` y el estado en
> `PLAN_REFACTORIZACION.md`. Equivalencias: `./scripts/*`→`UR/…`,`MiR/…`; el workspace
> ya es **`robot_workspace/`** (copia autocontenida, antes `/home/lab/pbd_system`);
> paquetes ROS en `robot_workspace/ros_ws/src/{UR_arms,end_effector,peripherals,shared_common}/`.
...
**Volumenes clave:**
```yaml
- ./robot_workspace:/root/workspace    # Copia autocontenida nuestra (antes /home/lab/pbd_system)
...
### Workspace ROS autocontenido (nuestra copia)

`./robot_workspace:/root/workspace` — copiamos el workspace (antes `/home/lab/pbd_system`, de Eemil) a `robot_workspace/` (gitignored) y lo montamos por ruta relativa. Sus paquetes (`duo_ur`, `moveit_utils_pkg`, `nordbo_ft_sensor`, etc.) ahora viven reorganizados en `robot_workspace/ros_ws/src/{UR_arms,end_effector,peripherals,shared_common}/`. El original de Eemil no se toca.
